	...
	for gen in range(n_iter):
		...
		for i in range(n_pop):
			if scores[i] < best_eval:
				...
				# print("[->] NFOB: %d [+] %d Gen,  new best f(%s) = %f" % (nfob, gen,  decoded[i], scores[i]))
				# print("[->] NFOB: %d [+] New best: %f [!] f(%s) = " %(nfob, scores[i], decoded[i]) )
				print("[->] NFOB: %d [+] New best: %f " % (nfob, scores[i]))
				dados.append(dados_parciais)
		probs = calc_probability_rws(pop, scores)
		selected = [roulette_wheel_selection(pop, probs) for _ in range(n_pop)]
		# selected = [tournament_selection(pop, scores) for _ in range(n_pop)]  # select parents
		children = list()
...
bounds = [[-600.0, 600.0],[-600.0, 600.0],[-600.0, 600.0],[-600.0, 600.0],[-600.0, 600.0],
		  [-600.0, 600.0],[-600.0, 600.0],[-600.0, 600.0],[-600.0, 600.0],[-600.0, 600.0]] # define range for input

# default parameters
n_iter = 1000 # n_inter = nfob/n_pop = 10^5/100 = 10^3 = 1000 - define the total iterations
n_bits = 140 # bits per variable
...
# 		r_cross = i
# 		best, score = genetic_algorithm(objective, bounds, n_bits, n_iter, n_pop, r_cross,r_mut)  # perform the genetic algorithm search
# 		print("[%.4f][r_cross] Done" %(i))
# 		execution.append(score)
# 	print("[%d] Execution finished" %(j))
...
n_pop = 160 # do cenario anterior
r_cross = 1
r_mut = 0.005
# r_cross = 1 # do cenario anterior
# for j in range(50):
# 	execution = list()
...
# 		start = time.time()
# 		best, score = genetic_algorithm(objective, bounds, n_bits, n_iter, n_pop, r_cross,r_mut)  # perform the genetic algorithm search
# 		print("[r_mut][%.4f] Done [->] Score: %.1f [->] Time: %.1f" %(i, score, time.time()-start))
# 		execution.append(score)
# 	print("[%d] Execution finished" %(j))
